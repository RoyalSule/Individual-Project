...
def summary_metrics(predictions, model_name):
    return {
        "Model": model_name,
...
def plot_equity_curve(predictions, model_name, data):
    # MA crossover — buy when 50-day MA crosses above 200-day MA
    ma50 = data['Close'].rolling(50).mean()
    ma200 = data['Close'].rolling(200).mean()
    ma_signal = (ma50 > ma200).astype(int)
    ma_returns = predictions['Returns'] * ma_signal.shift(1).reindex(predictions.index)
    ...
    ma_returns.cumsum().plot(ax=ax, color='green')
    ax.set_xlabel('Date')
...
def plot_prediction_distribution(predictions, model_name):
    ...
    predictions['Predictions'].value_counts().sort_index().plot.bar(
        ax=axes[0], color=['tomato', 'mediumseagreen'],
        title='Predicted: 0 vs 1', rot=0
    )
    predictions['Target'].value_counts().sort_index().plot.bar(
        ax=axes[1], color=['tomato', 'mediumseagreen'],
        title='Actual: 0 vs 1', rot=0
    )
